...
import org.openurp.prac.activity.model.{PracClazz, PracClazzSchedule}
import org.openurp.starter.web.support.ProjectSupport

import java.time.temporal.ChronoUnit
import java.time.{Duration, LocalDate}

class PracClazzScheduleAction extends RestfulAction[PracClazzSchedule], ProjectSupport {
  override def search(): View = {
    val query = super.getQueryBuilder
    get("pracClazzSchedule.activity.id") match {
      case Some(id) =>
        query.limit(null)
        ...
        put("pracClazzSchedules", schedules)
        forward()
      case None =>
        super.search()
    }
  ...
  override protected def editSetting(schedule: PracClazzSchedule): Unit = {
    given project: Project = getProject

    schedule.activity = entityDao.get(classOf[PracClazz], schedule.activity.id)
  ...
  override protected def saveAndRedirect(schedule: PracClazzSchedule): View = {
    val activity = entityDao.get(classOf[PracClazz], schedule.activity.id)
    schedule.teachers.clear();
    schedule.teachers ++= entityDao.find(classOf[User], getAll("teacherUserId", classOf[Long]))

    val newSchedules = Collections.newBuffer[PracClazzSchedule]
