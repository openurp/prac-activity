...
[/#list]
[/#macro]
[@b.grid items=pracClazzes var="pracClazz"]
  [@b.gridbar]
  ...
  [@b.row]
    ...
    [@b.col width="15%" property="courseName" title="课程名称"]
      <span title="${(pracClazz.courseType.name)!}">${pracClazz.courseName}</span>
    [/@]
    [@b.col property="department.name" width="60px" title="开课院系"]
      [@display pracClazz.department.shortName!pracClazz.department.name 60/]
    [/@]
    [@b.col width="10%" title="授课教师"]
      [#list pracClazz.teachers as t]${t.name}[#sep]&nbsp;[/#list]
      ${(pracClazz.externTeacher)!}
    [/@]
